...
- **Instructors:** can view their past appointments, but cannot change the
  booking or its status after the date has passed.

The same rule is enforced by the server, so it cannot be bypassed by refreshing
the page or calling a calendar URL directly.
